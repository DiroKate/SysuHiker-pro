...
const getMyRoutes = (path, routerData) => {
  const keys = Object.keys(routerData).filter(routePath => routePath !== '/');
  return keys.map((item) => {
    return {
      key: item,
      path: item,
      component: routerData[item].component,
      exact: routerData[item].exact || true,
    };
  });
};
...
class MyLayout extends React.PureComponent {
  ...
  render() {
    ...
    const myRoutes = getMyRoutes(match.path, routerData);
    const layout = (
      <Layout>
        <MySiderMenu
          collapsed={collapsed}
